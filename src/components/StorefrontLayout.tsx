...
export default function StorefrontLayout() {
  ...
  const tgValue = tgSetting?.value || 'Belksonshop'
  ...
  const tgChannelUrl = tgValue.startsWith('http')
    ? tgValue
  ...
  return (
    <>
      ...
      {/* ── Original Centered Clean Footer ────────────────────────── */}
      <footer className="bg-surface-container-low w-full px-margin-mobile md:px-margin-desktop py-12 flex flex-col items-center gap-6 max-w-[1200px] mx-auto border-t border-surface-dim mt-12 box-border">
        <h2 className="font-display-lg-mobile text-primary font-bold tracking-tight text-3xl">
          Belkson
        </h2>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#2AABEE]/10 hover:bg-[#2AABEE]/20 text-[#2AABEE] text-xs font-semibold transition-all cursor-pointer shadow-2xs hover:scale-105 active:scale-95"
        >
        ...
        <nav className="flex flex-wrap justify-center items-center gap-x-8 gap-y-3 font-body-md text-xs sm:text-sm">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-on-surface-variant hover:text-[#2AABEE] transition-colors font-medium flex items-center gap-1.5"
          >
            <svg className="w-4 h-4 fill-current text-[#2AABEE]" viewBox="0 0 24 24">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.07-.2-.08-.06-.19-.04-.27-.02-.12.02-1.96 1.25-5.54 3.69-.52.36-1 .53-1.42.52-.47-.01-1.37-.26-2.03-.48-.82-.27-1.47-.42-1.42-.88.03-.25.38-.51 1.07-.78 4.18-1.82 6.97-3.02 8.37-3.61 3.98-1.66 4.81-1.95 5.35-1.96.12 0 .38.03.55.17.14.12.18.28.2.45-.02.07-.02.16-.04.24z"/>
            </svg>

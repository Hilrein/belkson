...
export interface FetchHMCatalogParams {
  region?: RegionId
  category?: string
  subcategory?: string
  size?: string
...
export interface FetchHMCatalogResult {
  products: Product[]
  totalCount: number
  hasMore: boolean
...
export const hmCatalogService = {
  async fetchHMCatalog(params: FetchHMCatalogParams): Promise<FetchHMCatalogResult> {
    ...
    const endpoint = `${apiUrl}/api/hm/catalog?${query.toString()}`

    console.log('Fetching H&M API:', endpoint)

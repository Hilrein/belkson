...
async function ensureShowcaseIndexes(client: NeonQueryFunction<false, false>) {
  if (showcaseIndexesEnsured) return
  try {
    await client`CREATE INDEX IF NOT EXISTS idx_products_is_new_active ON products (id DESC) WHERE is_new = true AND status <> 'Нет в наличии'`
    await client`CREATE INDEX IF NOT EXISTS idx_products_is_favorite_active ON products (id DESC) WHERE is_favorite = true AND status <> 'Нет в наличии'`
    showcaseIndexesEnsured = true
  } catch (err) {
    console.warn('ensureShowcaseIndexes error:', err)
  }
}
...
async function handleCatalogRequest(c: any) {
  ...
  if (isNew || isFav) await ensureShowcaseIndexes(client)
  ...
  const [countRows, products, settings] = await Promise.all([
    client.query(countSql, countParams) as Promise<{ total: number }[]>,
    client.query(selectSql, params) as Promise<DbProduct[]>,
...
async function ensureHeroBannersTable() {
  ...
  // Таблица уже есть — добиваем недостающие колонки (для старых БД без updated_at и т.д.)
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS badge TEXT NOT NULL DEFAULT ''`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS title TEXT NOT NULL DEFAULT ''`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS subtitle TEXT NOT NULL DEFAULT ''`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS image TEXT NOT NULL DEFAULT ''`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS button_text TEXT NOT NULL DEFAULT 'В каталог'`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS button_url TEXT NOT NULL DEFAULT '/catalog'`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 0`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`
  await client`ALTER TABLE hero_banners ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`
}
